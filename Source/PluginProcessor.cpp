...
/Users/johnevans/Documents/DelayPlugIn/Source/PluginProcessor.cpp
 ==============================================================================
 */
...
#endif
                  ), parameters(*this, nullptr, Identifier("Background1"),
                                {
                                    // list of parameters and their values within the plugin
                                    std::make_unique<AudioParameterFloat>("gainparam", // parameter ID
                                                                          "gainparam", // parameter name
                                                                          0.0f, // min value
                                                                          100.0f, // max value
                                                                          50.0f // default value
                                                                          ),
                                    std::make_unique<AudioParameterFloat>("timeparam",
                                                                          "Timeparam",
                                                                          10.0f,
                                                                          2000.0f,
                                                                          500.0f
                                    ...
                                    std::make_unique<AudioParameterFloat>("feedbackparam",
                                                                          "Feedbackparam",
                                                                          0.0f,
                                                                          100.0f,
                                                                          50.0f
...
#endif
{
    // stores pointers to parameters
    gainParameter = parameters.getRawParameterValue("gainparam");
    timeParameter = parameters.getRawParameterValue("timeparam");
    feedbackParameter = parameters.getRawParameterValue("feedbackparam");
}
...
{
    // added smoothing
    float gain = *gainParameter / 100.0f;
    gainLevel.reset(mSampleRate, 0.05f);
    gainLevel.setTargetValue(gain);
    
    // main buffer data copied to delay buffer
    if (delayBufferLength > bufferLength + mWritePosition)
    {
        mDelayBuffer.copyFromWithRamp(channel, mWritePosition, bufferData, bufferLength, gainLevel.getNextValue(), gainLevel.getNextValue());
    }
    else {
        const int bufferRemaining = delayBufferLength - mWritePosition;
        // remaining samples copied to end of delay and repeated
        mDelayBuffer.copyFromWithRamp(channel, mWritePosition, bufferData, bufferRemaining, gainLevel.getNextValue(), gainLevel.getNextValue());
        mDelayBuffer.copyFromWithRamp(channel, 0, bufferData, bufferLength - bufferRemaining, gainLevel.getNextValue(), gainLevel.getNextValue());
    }
}
...
void DelayPluginAudioProcessor::getFromDelayBuffer (AudioBuffer<float>& buffer, int channel, const int bufferLength, const int delayBufferLength, const float* bufferData, const float* delayBufferData)
{
    ...
    timeLevel.reset(mSampleRate, 0.05f);
    timeLevel.setTargetValue(delayTime);
    ...
    {
        buffer.addFrom(channel, 0, delayBufferData + readPosition, bufferLength, gain);
    }
    else {
        const int bufferRemaining = delayBufferLength - readPosition;
        buffer.copyFrom(channel, 0, delayBufferData + readPosition, bufferRemaining, dry);
...
void DelayPluginAudioProcessor::feedbackDelay (int channel, const int bufferLength, const int delayBufferLength, float* dryBuffer)
{
    ...
    feedbackLevel.reset(mSampleRate, 0.05f);
    feedbackLevel.setTargetValue(feedback);
    
    if (delayBufferLength > bufferLength + mWritePosition)
    {
        mDelayBuffer.addFromWithRamp(channel, mWritePosition, dryBuffer, bufferLength, feedbackLevel.getNextValue(), feedbackLevel.getNextValue());
    }
    else
    {
        const int bufferRemaining = delayBufferLength - mWritePosition;
        
        mDelayBuffer.addFromWithRamp(channel, bufferRemaining, dryBuffer, bufferRemaining, feedbackLevel.getNextValue(), feedbackLevel.getNextValue());
        mDelayBuffer.addFromWithRamp(channel, 0, dryBuffer, bufferLength - bufferRemaining, feedbackLevel.getNextValue(), feedbackLevel.getNextValue());
    }
}
